...
class CppTemplateKernel(CppKernel):
    ...
    def store_pointwise_nodes(
        ...
        if len(reindexers) == 1 and reindexers[0] is not None:
            ...
            two_d_gemm_index_without_offset = var_ranges.keys()
            two_d_gemm_index = [
                arg + offset
                for arg, offset in zip(two_d_gemm_index_without_offset, offsets)
            ]
            six_d_gemm_index = reshape_reindex(
                two_d_gemm_index
            )  # From 2D GEMM out to 6D GEMM out
            # transpose
